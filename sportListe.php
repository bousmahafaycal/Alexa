...
$retour_total=$bdd->query('SELECT COUNT(*) AS total FROM Article WHERE idCategorie=5 AND affiche=1 AND invisibleAuteur=0'); //Nous récupérons le contenu de la requête dans $retour_total
...
if(isset($_GET['page'])) // Si la variable $_GET['page'] existe...
{
     ...
     if($pageActuelle>$nombreDePages) // Si la valeur de $pageActuelle (le numéro de la page) est plus grande que $nombreDePages, on ramene a la derniere page...
     {
          ...
          /* $pageActuelle=$nombreDePages; */                    /* --> redirige vers la derniere page possible */
     }
}
else // Sinon
{
...
<html lang="fr">
    ...
    <body>
      ...
      <div class="category_banner">
        <div class="container">
          <h1>Sport (Liste de tous les articles)</h1>
        </div>
      </div>
          ...
          <div class="left">
            <div class="articles">
              ...
              <?php 
                ...
                $retourmessages = $bdd->prepare('SELECT id, idAuteur, titre, idCategorie, corps, DATE_FORMAT(date_creation, \'%d/%m/%Y\') AS date_creation_fr FROM Article WHERE idCategorie=5 AND affiche=1 AND invisibleAuteur=0 ORDER BY date_creation DESC LIMIT '.$premiereEntree.', '.$messagesParPage.'');
