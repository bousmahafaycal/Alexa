...
<html lang="fr">
    ...
    <body>
        ...
        <section id="part_index"> <!-- peut etre à enlever si je laisse une seule section -->
          <div class="left">
            <div class="last_article">
              ...
              <?php
                $req = $bdd->query('SELECT id, idAuteur, titre, idCategorie, corps, DATE_FORMAT(date_creation, \'%d/%m/%Y à %Hh%i\') AS date_creation_fr FROM Article WHERE idCategorie=5 AND affiche=1 AND invisibleAuteur=0 ORDER BY date_creation DESC LIMIT 0, 1');
              ...
              <article>
                <a class="main_link" href="article.php?billet=<?php echo ($donnees['id']); ?>" style="text-decoration: none;">
                  <h2><?php echo htmlspecialchars($donnees['titre']) ?>
                    ...
                    if ($art->getGratuit()==false && ! ArticlePaye::existeArticlePaye($pers->getId(),$art->getId(),$bdd)) { ?>
            ...
            <div class="articles">
              ...
              <?php 
                $req = $bdd->query('SELECT id, idAuteur, titre, idCategorie, corps, DATE_FORMAT(date_creation, \'%d/%m/%Y\') AS date_creation_fr FROM Article WHERE idCategorie=5 AND affiche=1 AND invisibleAuteur=0 ORDER BY date_creation DESC LIMIT 1, 4');
      ...
      <div class="container accesListe">
        <a class="sinslink_bis linkListe" href="sportListe.php">Acceder à la liste de tous les articles de cette catégorie</a>
      </div>
