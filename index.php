...
<html lang="fr">
    ...
    <body>
        ...
        <section id="part_index"> <!-- peut etre à enlever si je laisse une seule section -->
          <div class="left">
            <div class="last_article">
              ...
              <?php
                $req = $bdd->query('SELECT id, idAuteur, titre, idCategorie, corps, DATE_FORMAT(date_creation, \'%d/%m/%Y à %Hh%i\') AS date_creation_fr FROM Article WHERE affiche=1 AND invisibleAuteur=0 ORDER BY date_creation DESC LIMIT 0, 1');
              ...
              <article>
                <a class="main_link" href="article.php?billet=<?php echo ($donnees['id']); ?>" style="text-decoration: none;">
                  ...
                  <figure>
                    <img src=<?php echo "\"uploadArticle/".$art->getImage()."\""; ?> alt="logo de l'article" title="logo de l'article" height="300" width="165"> <!-- 420x232px et 300x165px -->
                  </figure>
                </a>
            ...
            <div class="articles">
              ...
              <?php 
                $req = $bdd->query('SELECT id, idAuteur, titre, idCategorie, corps, DATE_FORMAT(date_creation, \'%d/%m/%Y\') AS date_creation_fr FROM Article WHERE affiche=1 AND invisibleAuteur=0 ORDER BY date_creation DESC LIMIT 1, 4');
            ...
            <div id="most_commented">
              <!-- A essayer - faire une boucle pour regarder les articles avec le plus de commentaires (dans le mois ? check si on peut gerer sur la durée) PHP -->
              <!-- template pour css -->
              <h3>Les plus commentés depuis toujours.</h3>
              <div class="listing_tjs">
                <article>
                <?php
                        
                        //$req = $bdd->prepare('SELECT Article.id ,COUNT(Commentaire.id) AS compteur   FROM `Article` INNER JOIN `Commentaire`  ON Commentaire.idArticle = Article.id ORDER BY compteur  DESC LIMIT 0, 1');
                        //$req = $bdd->prepare('SELECT a.id   FROM `Article` a INNER JOIN (SELECT ) b   ON Commentaire.idArticle = Article.id ORDER BY compteur  DESC LIMIT 0, 1');
                        //$req = $bdd->prepare('SELECT Article.id AS compteur   FROM (SELECT COUNT(Commentaire.id) AS compteur   FROM `Article` INNER JOIN `Commentaire`  ON Commentaire.idArticle = Article.id  ) INNER JOIN `Commentaire`  ON Commentaire.idArticle = Article.id ORDER BY compteur  DESC LIMIT 0, 1');
                        $req = $bdd->prepare
                        ('SELECT a.id, tmp.nbrComm FROM Article a
                        INNER JOIN (
                          SELECT idArticle, COUNT(*) nbrComm FROM Commentaire
                          GROUP BY idArticle
                        ) tmp 
                        ON a.id = tmp.idArticle AND a.affiche=1 AND a.invisibleAuteur=0
                        ORDER BY nbrComm DESC
                        LIMIT 0, 1');
                        $req->execute();
                        $donnees = $req->fetch();
                        $req->closeCursor(); 
                        $art = new Article($donnees[0], $bdd); 
                     ?>
                  <figure>
                    <a  href="article.php?billet=<?php echo ($art->getId()); ?>">
                  ...
                  <h2>
                    <a href="article.php?billet=<?php echo ($art->getId()); ?>" title="Voir l'article"> <!-- même balise que ci-dessus -->
                      <?php echo $art->getTitre(); ?>
                    </a>
                  </h2>
                  <a href="article.php?billet=<?php echo ($art->getId()); ?>" class="comments">
                    <?php echo ($art->getNbCommentaire()); ?> <!-- nbres de comments + ajouter l'id #comments (ou autre) dans le href pour aller direct aux commentaires et pas slmt la page -->
                  </a>
                </article>
                <article>
                   <?php
      ...
      <div class="container accesListe">
        <a class="sinslink_bis linkListe" href="articlesListe.php">Acceder à la liste de tous les articles</a>
      </div>
